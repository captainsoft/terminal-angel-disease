...
/**
 * A group of combined, rectangular tile images. This usually exists once per replace tile,
...
public final class TileGroup {
    ...
    public TileGroup(Surface surface, int anchor_x, int anchor_y) {
        ...
        //
        x_span = surface.width() / c;
        y_span = surface.height() / c;
        //
        imageListZickZackOrder = new ArrayList<TileGroupImage>();
        tiles = new TileGroupImage[x_span][y_span];
        for (int y = 0; y < y_span; y++) {
            for (int x = 0; x < x_span; x++) {
                Image tileImage = surface.stamp(x * c, y * c, c, c).image();
                TileGroupImage tgi = new TileGroupImage(new Surface(tileImage));
                tgi.name = x + " " + y;
                tiles[x][y] = tgi;
    ...
    public void setOverlayTiles(ImageFilter filter, Integer... overlays) {
        for (int o : overlays) {
            ...
            tip.asOverlay();
            tip.image(new Surface(ImageTool.createFiltered(tip.image().image(), filter), true));
        }
    }
    ...
    public void setSecondOverlayTiles(Integer... secondOverlays) {
        for (int o : secondOverlays) {
            TileGroupImage tip = infoAt(o);
            tip.asSecondOverlay();
            tip.image(new Surface(ImageTool.createFiltered(tip.image().image(), colorFilter), true));
        }
    }
    ...
    public String toString() {
        return Utils.stringer("TileGroup", name);
    }
